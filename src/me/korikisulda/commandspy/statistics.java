...
public class statistics {
    ...
    public void startconnection(String host, Integer port,String username,String password, String database){	
		mysql = new MySQL(plugin.log, "[CommandSpy]",host,port.toString(),database, username, password);
			 try {
		            mysql.open();
	...
	public void addcommand(loggedcommand Log){
		int status=0;
		int westatus=0;
		if(Log.denied) status=1; else status=0;
		if(Log.isWorldedit) westatus=1; else westatus=0;
		if(plugin.getServer().getPluginCommand(Log.getCommandStripped())==null) status=2;
		if(!useStats) return;
		try{
		mysql.query("INSERT INTO commandspy (cmdBase, cmdExt, username, usergroup, userDest, world, cmdStatus, isWE) VALUES	('" + Log.command + "','"+ Log.arguments + "','" + Log.playername + "','" + Log.groupname +"','" + Log.location.getBlockX() + " " + Log.location.getBlockY() + " " + Log.location.getBlockZ() + "','" + Log.world + "'," + status + "," + westatus + ");").close();
		if (Log.isWorldedit) mysql.query("INSERT INTO commandspy_we VALUES (LAST_INSERT_ID()," + Log.selection.getMinimumPoint().getBlockX() + "," + Log.selection.getMinimumPoint().getBlockY() + "," + Log.selection.getMinimumPoint().getBlockZ() + "," + Log.selection.getMinimumPoint().getBlockX() + "," + Log.selection.getMinimumPoint().getBlockY() + "," + Log.selection.getMinimumPoint().getBlockZ() + ");").close();
		}catch(Exception ex){
			//ex.printStackTrace();
		}
		}
	...
	public ArrayList<loggedcommand> getWorldEdits(Location loc){
		...
		ResultSet rs=mysql.query("SELECT * FROM commandspy JOIN commandspy_we ON commandspy.id = commandspy_we.id WHERE IsWE = 1 AND ((x1 <= " + loc.getBlockX() + " <= x2) OR (x2 <= " + loc.getBlockX() + " <= x1)) AND ((y1 <= " + loc.getBlockY() + " <= y2) OR (y2 <= " + loc.getBlockY() + " <= y1)) AND ((z1 <= " + loc.getBlockZ() + " <= z2) OR (z2 <= " + loc.getBlockZ() + " <= z1));");
		try {
			
			while ( rs.next() ) {
				loggedcommand l=new loggedcommand(plugin);
				//cmdBase, cmdExt, username, usergroup, userDest, world, cmdStatus, isWE
				...
				l.DBselection=new CuboidSelection(world, new Location(world,0F,0F,0F), new Location(world,0F,0F,0F));
				l.isWorldedit=true;
				l.location=null;
